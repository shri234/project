...
const BuyTicket: FC<{ name: string; path: string }> = ({ name, path }) => {
  ...
  const decreaseCount = () => {
    ...
    if(walletamount>ticket_price){
     const response=await axios.post(`http://localhost:3002/ticket/addTicketCount?userId=${sessionStorage.getItem("userId")}`, body);
     ...
     if(response.status==200){
      ...
      try{
        ...
        const res=await axios.post(`http://localhost:3002/ticket/addWalletAmount`, body);
      }
      catch(err){
      }
...
,[]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3002/ticket/getTicketRate?date=${datee}`,{
          headers: {
            'Content-Type':"application/json"
          }
        });
